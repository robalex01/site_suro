...
}: { selected?: string, onSelect?: (ws: string) => void, workspaces?: string[] }) {
...
export function SidebarNav({
  ...
  // ── Adapted for real integration (not in the original snippet) ──────────
  // The original component always renders a hardcoded WorkspaceSwitcher
  // (3 mock workspace names). This app has no "workspace" concept, so a
  // real caller passes its own `header`/`footer` (brand + user box) and
  // the switcher is skipped entirely — the demo/preview below keeps using
  // it exactly as given, since it passes neither.
  header,
  footer,
}: {
  className?: string,
...
/**
 * Standalone preview of the component (used by `npm run dev` in this
 * folder only) — exactly the demo/preview from the original snippet,
 * unchanged. Not imported anywhere in the real Access Panel integration;
 * see ../../AppSidebar.tsx for that.
 */
