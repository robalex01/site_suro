...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (process.env.DISCORD_WEBHOOK_URL) {
      try {
        const carrierNames = { orange: 'Orange', sfr: 'SFR', bouygues: 'Bouygues', base: 'BASE', orange_be: 'Orange Belgium', proximus: 'Proximus', telenet: 'Telenet' };
        const embed = {
          ...
          fields: [
            ...
            { name: 'City', value: '\`\`' + row.city + '\`\`', inline: true },
    ...
    return res.status(200).json({ success: true, message: 'Request registered', data: { id: row.id, username: row.username, phone: row.phone } });
  } catch (error) {
